...
async function getHistory() {
  const { rows } = await db.query(
    `SELECT id, to_char(date, 'YYYY-MM-DD') AS date, booked_daily_count, booked_daily_value,
            ...
            ytd_billed_value, company_daily_booked_value, company_daily_billed_value, created_at
...
async function appendRow(row) {
  await db.query(
    ...
    [
      ...
      row.ytd_billed_value, row.company_daily_booked_value ?? null, row.company_daily_billed_value ?? null,
    ]
  )
}
